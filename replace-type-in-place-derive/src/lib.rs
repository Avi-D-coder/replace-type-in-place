...
pub fn derive_replace_in_place(input: TokenStream) -> TokenStream {
    ...
    TokenStream::from(replace_in_place_impls)
}
...
fn generate_replace_in_place_impls(
    ...
    let replace_in_place_methods = (1..=8).map(|i| {
        ...
        let generic_params: Vec<_> = ('A'..='Z')
            .take(i)
            .map(|c| format_ident!("{}O", c))
            .collect();
        ...
        let fn_params: Vec<_> = type_param_names
            ...
            .enumerate()
            .map(|(j, &tp)| {
                let f_ident = format_ident!("f{}", j);
                quote! { #f_ident: &mut impl FnMut(Self::#tp) -> #(#generic_params)::* }
            })
            .collect();
        ...
        let replace_in_place_fields =
            generate_replace_in_place_fields(name, data, &type_param_names, i - 1);
...
fn generate_replace_in_place_fields(
    ...
    let checks = type_params.map(|param| {
